...
import (
	...
	laddr "github.com/OpenBazaar/multiwallet/gleecbtc/address"
...
var _ = wi.Wallet(&GleecbtcWallet{})
...
func (w *GleecbtcWallet) IsDust(amount int64) bool {
...
func (w *GleecbtcWallet) Balance() (confirmed, unconfirmed int64) {
...
func (w *GleecbtcWallet) GetFeePerByte(feeLevel wi.FeeLevel) uint64 {
	return w.fp.GetFeePerByte(feeLevel)
}

func (w *GleecbtcWallet) Spend(amount int64, addr btcutil.Address, feeLevel wi.FeeLevel, referenceID string, spendAll bool) (*chainhash.Hash, error) {
	var (
		tx  *wire.MsgTx
		err error
	)
...
func (w *GleecbtcWallet) EstimateFee(ins []wi.TransactionInput, outs []wi.TransactionOutput, feePerByte uint64) uint64 {
	tx := new(wire.MsgTx)
...
func (w *GleecbtcWallet) Broadcast(tx *wire.MsgTx) error {
	...
	for n, in := range tx.TxIn {
		...
		input := model.Input{
			...
			Addr:     addr.String(),
			Satoshis: u.Value,
			Value:    float64(u.Value) / util.SatoshisPerCoin(wi.Gleecbtc),
		}
