...
func (w *LitecoinWallet) CurrentAddress(purpose wi.KeyPurpose) wi.WalletAddress {
	key, _ := w.km.GetCurrentKey(purpose)
	addr, _ := litecoinAddress(key, w.params)
	return btcutil.Address(addr)
}

func (w *LitecoinWallet) NewAddress(purpose wi.KeyPurpose) wi.WalletAddress {
...
func (w *LitecoinWallet) DecodeAddress(addr string) (wi.WalletAddress, error) {
...
func (w *LitecoinWallet) AddressToScript(waddr wi.WalletAddress) ([]byte, error) {
	addr, _ := btcutil.DecodeAddress(waddr.String(), w.params)
	return laddr.PayToAddrScript(addr)
}

func (w *LitecoinWallet) HasKey(waddr wi.WalletAddress) bool {
	addr, _ := btcutil.DecodeAddress(waddr.String(), w.params)
...
func (w *LitecoinWallet) Spend(amount int64, waddr wi.WalletAddress, feeLevel wi.FeeLevel) (*chainhash.Hash, error) {
	addr, _ := btcutil.DecodeAddress(waddr.String(), w.params)
...
func (w *LitecoinWallet) EstimateFee(ins []wi.TransactionInput, outs []wi.TransactionOutput, feePerByte uint64) uint64 {
	...
	for _, out := range outs {
		...
		scriptPubKey, _ := laddr.PayToAddrScript(btcAddr)
...
func (w *LitecoinWallet) SweepAddress(ins []wi.TransactionInput, waddress *wi.WalletAddress, key *hd.ExtendedKey, redeemScript *[]byte, feeLevel wi.FeeLevel) (*chainhash.Hash, error) {
...
func (w *LitecoinWallet) AddWatchedAddress(waddr wi.WalletAddress) error {
	addr, _ := btcutil.DecodeAddress(waddr.String(), w.params)
